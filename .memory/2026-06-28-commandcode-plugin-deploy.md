...
- Direct Volcengine Ark comparison, bypassing New API: `https://ark.cn-beijing.volces.com/api/coding/v1/chat/completions` with `glm-5.2` returned HTTP 200 for non-streaming and streaming requests. Direct Volcengine non-streaming response included `message.reasoning_content`; direct Volcengine streaming response included `delta.reasoning_content` in 11 chunks. Direct `10.0.8.9:8317` Command Code responses for the same prompt returned HTTP 200, no `reasoning_content`, `content`/`role` only in assistant output, one stream id, one finish chunk, one usage chunk, `[DONE]`, no malformed SSE, and OpenAI-compatible usage totals. Comparison marker: `direct-volc-8987557f`.
- Claude review pass: Polycli Claude reviewed diff `56ff4f54..6e892d19` for strict OpenAI response normalization and reported no blocking findings. Non-blocking observations were adjudicated against source: production streaming uses one persistent `commandCodeStreamConverter` in `streamCommandCodeNDJSONToHost`, so the one-shot helper does not duplicate role chunks in production; non-streaming finish reasons are normalized by `ApplyLine` via `mapFinishReason`. Residual edge risks: if Command Code ever omits `toolName` on `tool-input-start` and sends it only later, the streaming start chunk cannot retroactively include the name; a pathological start/end tool stream with no deltas and no fallback `tool-call.input` would leave streamed arguments empty. These do not match the documented Command Code event sequence and did not block deploy.
