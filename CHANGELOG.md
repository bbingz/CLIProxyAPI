...
- Verified on `10.0.8.9`: `/v1/models` HTTP 200 with Command Code models `deepseek/deepseek-v4-flash` and `deepseek/deepseek-v4-pro`, Grok count 12, Gemini count 8, Command Code non-streaming smoke HTTP 200/content `OK`, Command Code streaming smoke HTTP 200/content `OK`/15 chunks/0 malformed chunks, and existing `zai-org/GLM-5.2` non-streaming smoke HTTP 200/content `OK`.
